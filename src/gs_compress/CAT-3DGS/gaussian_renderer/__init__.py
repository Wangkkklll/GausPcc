...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False, step=0, mask=None):
    ...
    binary_grid_masks = pc.get_mask(mask)[visible_mask]
    mask_anchor = pc.get_mask_anchor(mask)[visible_mask]
...
####OURS
        ...
        if step > 10000 or step==-1:
            ...
            for i in range(len(list_of_means)):
                if i == 0:
                    bit_feat = pc.entropy_gaussian.forward(feat_list[i], list_of_means[i], list_of_scales[i], Q_feat, pc._anchor_feat.mean())
                    bit_feats = bit_feat
                    decoded_feat = feat_list[i]
                else:
                    dmean, dscale = torch.split(pc.get_chcm_mlp_list[i-1](decoded_feat), split_size_or_sections=[pc.chcm_slices_list[i], pc.chcm_slices_list[i]], dim=-1)
                    mean = list_of_means[i] + dmean
                    scale = list_of_scales[i] + dscale
                    bit_feat = pc.entropy_gaussian.forward(feat_list[i], mean, scale, Q_feat, pc._anchor_feat.mean())
                    bit_feats = torch.cat([bit_feats, bit_feat], dim=-1)
                    decoded_feat = torch.cat([decoded_feat, feat_list[i]], dim=-1)

            if pc.chcm_for_scaling:
                dmean_scaling, dscale_scaling = torch.split(pc.get_chcm_mlp_scaling(decoded_feat), split_size_or_sections=[6, 6], dim=-1)
            ...
            if pc.chcm_for_offsets:
                dmean_offsets, dscale_offsets = torch.split(pc.get_chcm_mlp_offsets(decoded_feat), split_size_or_sections=[3*pc.n_offsets, 3*pc.n_offsets], dim=-1)
        ...
        if step >= 10000 or step==-1:
            ...
            mean1, scale1, mean2, scale2, mean_scaling, scale_scaling, mean_offsets, scale_offsets, Q_feat_adj, Q_scaling_adj, Q_offsets_adj = \
                torch.split(feat_context, split_size_or_sections=[pc.feat_dim//2, pc.feat_dim//2, pc.feat_dim//2, pc.feat_dim//2, 6, 6, 3*pc.n_offsets, 3*pc.n_offsets, 1, 1, 1], dim=-1)
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, visible_mask=None, retain_grad=False, step=0, mask=None):
    """
    Render the scene.
    ...
    if is_training:
        xyz, color, opacity, scaling, rot, neural_opacity, mask, bit_per_param, bit_per_feats_param, bit_per_scaling_param, bit_per_offsets_param, feat_rate = generate_neural_gaussians(viewpoint_camera, pc, visible_mask, is_training=is_training, step=step, mask=mask)
    else:
        xyz, color, opacity, scaling, rot, time_sub, feat_rate = generate_neural_gaussians(viewpoint_camera, pc, visible_mask, is_training=is_training, step=step, mask=mask)
